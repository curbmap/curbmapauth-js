...
function sendAuth(transporter, username, email, authToken) {
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    ...
    winston.log('main:sendAuth', `sending mail to: ${email}`, {
...
function passwordMeetsCriteria(password) {
  return (password.length >= 9 && password.length <= 40 && password.match(passwordSpecial) && password.match(passwordCapital) && password.match(passwordLower) && password.match(passwordNum));
}
